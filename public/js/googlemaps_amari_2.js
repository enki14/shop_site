...
function initMap(){
    ...
    let service_2 = new google.maps.places.PlacesService(myMap);
    service_2.search(request_2, Result_Places);
}
...
function SearchGo(){
    ...
    let eriaword = document.getElementById("search-text");
    let request = {
        query: eriaword.value,
        radius: 5000,
        location: myMap.getCenter()
    };
    service.textSearch(request_2, result_search);
...
function result_search(results, status){
    ...
    results.forEach(function(val, i){
        ...
        if(i == results.length - 1){
            ...
            createMarker({
                // icon: 'img/IMG_0224.JPG',
                position: latlng,
                // text: '<span style="font-weight: bold;">我が家</span><br>住所',
                map: myMap
            });
        }else{
            createMarker({
